...
private:
    ...
    std::priority_queue<State<T>*> _vertex_queue;
    vector<State<T>*> _vectorQ;
    bool isAdjInPriorityQuque(State<T>*);
...
void BestFirstSearch<T>::eraseFromPriorityQueqe(int index) {
    priority_queue<State<T>*> newPriority;
    _vertex_queue = newPriority;
...
template <class T>
bool BestFirstSearch<T>::isAdjInPriorityQuque(State<T>* stateCompare) {
    bool isExists = false;
...
vector<State<T>*> BestFirstSearch<T>::search(Isearchable<T>* searchable) {
    ...
    while(!_vertex_queue.empty()){
        ...
        for (int j = 0; j < adj.size(); j++) {
            // adj is not in CLOSED and s is not in OPEN:
            if (!isAdjInClosedList(adj[j]) && !isAdjInPriorityQuque(adj[j])){
